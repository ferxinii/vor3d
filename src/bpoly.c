...
static int projection_lies_in_face(const s_point face[3], s_point normal, double d_plane, s_point p, double EPS_degenerate, s_point *out_projection)
{
    s_point mirror = mirror_plane(normal, d_plane, p);
    s_point segment[2] = {p, mirror};

    s_segment_intersect intersect = segment_triangle_intersect_3D(segment, face, EPS_degenerate, 0);
    if (intersect.type == INTERSECT_NONDEGENERATE || intersect.type == INTERSECT_DEGENERATE) {
        assert(intersect.N == 1);
        *out_projection = intersect.coords[0];
        return 1;
    } else return 0;
}

static int witness_points_from_face(const s_points *seeds, const s_point face[3], s_point normal, double d_plane, double EPS_degenerate, s_point_list *out_witness)
{   /* 0 ERROR, 1 OK */
    int N = 3;
    if (!increase_memory_point_list_if_needed(out_witness, N)) return 0;
    out_witness->list[0] = face[0];  out_witness->list[1] = face[1];  out_witness->list[2] = face[2];

    for (int ii=0; ii<seeds->N; ii++) {
        s_point projection;
        if (projection_lies_in_face(face, normal, d_plane, seeds->p[ii], EPS_degenerate, &projection)) {
            if (!increase_memory_point_list_if_needed(out_witness, N+1)) return 0;
            out_witness->list[N++] = projection;
        }
    }
    // printf("DEBUG: witness->N = %d\n", N);
    out_witness->N = N;
    return 1;
}

static int mirror_seeds_face(const s_points *seeds, const s_point face[3], s_point normal, double d_plane, double EPS_degenerate, bool buff_mask[seeds->N], s_point_list *buff_witness, s_point_list *out_mirrored)
{   /* 0 ERROR, 1 OK */
    memset(buff_mask, 0, sizeof(bool) * seeds->N);
    
    if (!witness_points_from_face(seeds, face, normal, d_plane, EPS_degenerate, buff_witness)) return 0;
    for (int ii=0; ii<buff_witness->N; ii++) {
        double dmin2 = distance_squared(buff_witness->list[ii], seeds->p[0]);
        int closest_id = 0;
        for (int jj=1; jj<seeds->N; jj++) {
            double d2 = distance_squared(buff_witness->list[ii], seeds->p[jj]);
            if (d2 < dmin2) {
                dmin2 = d2;
                closest_id = jj;
            }
        ...
        if (!buff_mask[closest_id]) {
            if (!increase_memory_point_list_if_needed(out_mirrored, out_mirrored->N+1)) return 0;
            out_mirrored->list[out_mirrored->N++] = mirror_plane(normal, d_plane, seeds->p[closest_id]);
        }
        buff_mask[closest_id] = true;
    }
    return 1;
}

int extend_sites_mirroring(const s_bpoly *bp, double EPS_degenerate, s_points *inout_seeds)
{   /* 0 ERROR, 1 OK */
    s_point_list mirrored = initialize_point_list(0);
    s_point_list buff_witness = initialize_point_list(0);
    bool *buff_mask_mirrored = calloc(inout_seeds->N, sizeof(bool));
    if (!mirrored.list || !buff_witness.list || !buff_mask_mirrored) return 0;
    
    /* Store all mirrored seeds */
    for (int ii=0; ii<bp->convh.Nf; ii++) {
        ...
        if (!mirror_seeds_face(inout_seeds, face, normal, d_plane, EPS_degenerate, buff_mask_mirrored, &buff_witness, &mirrored)) goto error;
    ...
    printf("DEBUG: mirrored->N = %d\n", mirrored.N);

    free_point_list(&mirrored);
    free_point_list(&buff_witness);
